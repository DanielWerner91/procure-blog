...
export function ArticleCard({
  ...
  const href = article.source_url || `/articles/${article.slug}`;
  const isExternal = !!article.source_url;

  const cardContent = (isFeaturedCard: boolean) => {
    if (isFeaturedCard) {
      return (
        <div className="grid md:grid-cols-2 gap-0">
          ...
          <div className="p-6 md:p-8 flex flex-col justify-center">
            ...
            <div className="mt-4 flex items-center gap-1.5 text-sm font-medium text-accent">
              {isExternal ? 'Read at source' : 'Read article'}
